...
type alias DrawActor = (Actor -> (Int,Int) -> Element)
--D.Dict String (Actor -> (Int,Int) -> Element)

type alias World2 = World {actions: D.Dict String Action, reactions: D.Dict String Reaction, draws: D.Dict String DrawActor}
...
stepWorld inp w = 
    let 
        ...
        (_,w2) = while 
                   ...
                   (0,w0) 
                   (\(i,wo) -> 
                       let a = getActor (getWithDefault 0 i wo.ilist) wo
--                         in (i+1, wo))
...
faceDict: List Element -> D.Dict Int Element
