...
type alias World a = {a | l:Int, h: Int, adict: D.Dict Int Actor, ilist: A.Array Int, alocs: D.Dict (Int,Int) (Set.Set Int), text:String, curId:Int, input:Input, info:DictInfo}
...
addActorAtLocs:Actor -> List (Int,Int) -> World a -> World a
addActorAtLocs ac li w = w |> foldlRot (\loc wo -> wo |> addActor (ac |> setLoc loc)) li

getActorsAt: (Int,Int) -> World {}-> List Actor
getActorsAt (x,y) w = 
    let 
        s = mget (x,y) w.alocs
    in
...
getAtSameSpace: (Actor, World {}) -> List Actor
getAtSameSpace (a,w) = getActorsAt (withDefault (0,0) (a.locs!0)) w
...
getSatisfyingAtSameSpace : (Actor -> Bool) -> (Actor, World {}) -> List Actor
...
type alias MoveCondition a = World a -> (Int,Int) -> Bool
